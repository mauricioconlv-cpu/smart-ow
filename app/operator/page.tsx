...
export default async function OperatorDashboard() {
  ...
  const { data: services } = await supabase
    ...
    .select(`
      ...
      costo_calculado, calidad_estrellas, firma_url,
      tipo_servicio, origen_coords, destino_coords,
      comentarios_calidad, clients ( name )
    `)
  ...
  // Service cards — pure HTML, no browser APIs, safe for SSR
    ...
    <div className="space-y-4">
      {services.map(service => (
        <div key={service.id} className="bg-white rounded-2xl p-5 shadow-sm border-l-4 border-blue-500 flex flex-col gap-4">
          ...
          {/* DownloadPDFButton rendered client-side via OperatorClientLayer for servicio_cerrado */}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <OperatorClientLayer
      ...
      services={services ?? []}
    >
      {/* Services section — passed as server-rendered children */}
