...
export default function ExportExcelButton({ data }: { data: any[] }) {
  ...
  const handleExport = () => {
    ...
    const formattedData = data.map(svc => ({
      ...
      'coord origen': svc.origen_coords?.address || [svc.origin_street, svc.origin_colonia, svc.origin_municipality, svc.origin_state].filter(Boolean).join(', ') || 'Sin Dirección',
      'coord destino': svc.destino_coords?.address || [svc.dest_street, svc.dest_colonia, svc.dest_municipality, svc.dest_state].filter(Boolean).join(', ') || 'Sin Dirección',
