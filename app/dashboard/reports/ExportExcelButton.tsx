...
export default function ExportExcelButton({ data }: { data: any[] }) {
  ...
  const handleExport = () => {
    ...
    const formattedData = data.map(svc => ({
      ...
      'coord origen': [svc.origin_street, svc.origin_colonia, svc.origin_municipality, svc.origin_state].filter(Boolean).join(', ') || svc.origen_coords?.address || 'Sin Dirección',
      'coord destino': [svc.dest_street, svc.dest_colonia, svc.dest_municipality, svc.dest_state].filter(Boolean).join(', ') || svc.destino_coords?.address || 'Sin Dirección',
